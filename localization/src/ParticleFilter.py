...
class ParticleFilter():
  ...
  def initialize_global(self):
    ...
    self.particles[:, 0] = 0#np.random.choice(permissible_x, size=self.particles.shape[0])
    self.particles[:, 1] = 0#np.random.choice(permissible_y, size=self.particles.shape[0])
    self.particles[:, 2] = 0#np.random.uniform(-np.pi, np.pi, size=self.particles.shape[0])
  ...
  def expected_pose(self):
    ...
    # Compute weighted mean of particle orientations using trigonometric averaging
    mean_cos_theta = np.average(np.cos(self.particles[:, 2]), weights=self.weights)
    mean_sin_theta = np.average(np.sin(self.particles[:, 2]), weights=self.weights)
  ...
  def clicked_pose_cb(self, msg):
    ...
    self.particles[:, 0] = np.random.normal(msg.pose.pose.position.x, .05, size=self.particles.shape[0])
    self.particles[:, 1] = np.random.normal(msg.pose.pose.position.y, .05, size=self.particles.shape[0])
    self.particles[:, 2] = np.random.normal(Utils.quaternion_to_angle(msg.pose.pose.orientation), .01, size=self.particles.shape[0])
    self.weights[:] = np.ones(self.particles.shape[0]) / float(self.particles.shape[0])
