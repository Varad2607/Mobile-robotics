...
# All Z_XXX values should add up to 1!
Z_SHORT = 0.25 # Weight for short reading
Z_MAX = 0.01 # Weight for max reading
Z_RAND = 0.10  # Weight for random reading
Z_HIT = 0.64 # Weight for hit reading

Z_NORM = np.sqrt(Z_SHORT**2 + Z_MAX**2 + Z_RAND**2 + Z_HIT**2)
Z_SHORT = Z_SHORT/Z_NORM
Z_MAX = Z_MAX/Z_NORM
Z_RAND = Z_RAND/Z_NORM
Z_HIT = Z_HIT/Z_NORM

SIGMA_HIT = 10.5 # Noise value for hit reading
...
class SensorModel:
  ...
  def lidar_cb(self, msg):
    ...
    downsampled_idx = np.array(range(0, len(msg.ranges), self.LASER_RAY_STEP))
    self.downsampled_angles= (msg.angle_min+(msg.angle_increment*downsampled_idx)).astype(np.float32) #http://docs.ros.org/en/melodic/api/sensor_msgs/html/msg/LaserScan.html

    downsampled_ranges= np.array(msg.ranges[::self.LASER_RAY_STEP]).astype(np.float32)
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    # Instead of applying the normalizing factor as described in the textbook, we just normalize each row. 
    # You should set the normalizing factor to 1 when required.
    variance = np.power(SIGMA_HIT, 2)
    norm = 1
    lambda_short = 0.001
    # true - ideal, noise-free measurement
    # real - noise measurement
    for true in range(table_width):
      row_sum = 0
      for real in range(table_width): 
        ...
        var = float(SIGMA_HIT)**2
        denom = (2*math.pi*var)**.5
        num = math.exp(-(float(real)-float(true))**2/(2*var))
        # print(r, d, float(num/denom), num, -(float(r)-float(d))**2/(2*var))
        # p_hit = float(num/denom) if r <= d else 0
        p_hit = float(num/denom)
        # print("phit",p_hit)
        # p_short = lambda_short * np.exp(-lambda_short * r) if r <= d else 0
        p_rand = float(real < int(max_range_px)) * 1 / int(max_range_px)
        p_short = lambda_short * np.exp(-lambda_short * real) if real <= true else 0
        p_max = 1.0 if real == max_range_px else 0
        # p_rand = 1.0 / max_range_px

        if true == 0:      
          norm = math.erf(float(max_range_px) / (math.sqrt(2) * SIGMA_HIT)) / 2
        else:
          norm = 1 / -np.expm1(-lambda_short * true)

        sensor_model_table[real, true] = Z_HIT * p_hit + Z_SHORT * p_short * norm + Z_MAX * p_max + Z_RAND * p_rand

      sensor_model_table[:, true] /= np.sum(sensor_model_table[:, true])
...
if __name__ == '__main__':
  ...
  bag_path = rospy.get_param("~bag_path", '/home/car-user/racecar_ws/src/ta_lab2/bags/laser_scans/laser_scan3.bag')
