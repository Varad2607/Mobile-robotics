...
def transform_from_relative_to_absolute(current_pose, relative_pose):
  
  delta_x, delta_y, delta_theta = relative_pose
  x, y, theta = current_pose
  absolute_x = x + delta_x * np.cos(theta) - delta_y * np.sin(theta)
  absolute_y = y + delta_x * np.sin(theta) + delta_y * np.cos(theta)
  absolute_theta = theta + delta_theta
  return np.array([absolute_x, absolute_y, absolute_theta])



def coordinate_transformation(planner_pose_config, car_pose):
  x_component = (planner_pose_config[0]-car_pose[0])*np.cos(car_pose[2])
  y_component = (planner_pose_config[1]-car_pose[1])*np.sin(car_pose[2])
  angle = x_component + y_component
  return angle